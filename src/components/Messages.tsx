...
const getContainerClass = (isBot?: boolean) =>
  `message-container ${isBot ? 'bot-message-container' : 'user-message-container'}`
...
const Message = ({ message, isBot }: Message) =>
  <div className={getContainerClass(isBot)}>
    <div className={getMessageClass(isBot)}>
      {message}
    </div>
  </div>
...
export default ({ conversation }: Props) =>
    <div id="tchatche-messages">
      {conversation.map(({ message, isBot }, i) => <Message key={i} message={message} isBot={isBot} />)}
